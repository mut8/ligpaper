...
par(#mfrow=c(4,2), 
  mar=c(4.1,5.1,2.1,.1), cex.axis=.8, cex.lab=1.1, cex=1.2, cex.main=1.2)  
...
plot(x,y, ylab=expression(Delta[LCI]), xlab="F:B protein abundance ratio", cex=.5, type="n", ylim=c(ymin,ymax), xlim=c(1,5))
plotCI(xmean, ymean, barcol=errcol, uiw=yse, liw=yse, add=T, gap=0, pch=pch, pt.bg=colscale, cex=2)
