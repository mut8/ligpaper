...
par(mfrow=c(3,2), mar=c(4.1,4.1,4.1,.1), cex.axis=.8, cex.label=1, cex=1.1, cex.main=1.1)  

par()

prot.means<-tapply(f2b$f2b_prot, list(alldata$days, alldata$Litter), mean)
plot(as.numeric(levels(as.factor(alldata$days))), prot.means[,1], xlim=c(0, 500), ylim=c(0, max(prot.means)), type="o", pch=pch[1], bg=colscale[1], xlab="Litter incubation (days)", main="Fungi/Bacteria (metaproteome)", ylab="F:B protein abundance ratio")
...
plot(x[1:19],y, ylab="", xlab="F:B DNA ratio", cex=.5, type="n", ylim=c(ymin,ymax), xlim=c(1,5), yaxt="n")
plotCI(xmean, ymean, barcol=errcol, uiw=yse, liw=yse, add=T, gap=0, pch=pch, pt.bg=colscale, cex=1.2)
axis(2, labels=F)
axis(3, labels=F)
axis(4)
